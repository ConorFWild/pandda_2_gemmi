...
from pandda_gemmi.scratch.dataset import XRayDataset, Reflections, StructureArray
...
def test_sparse_dmap_stream(data_dir, out_dir):
    ...
    alignment_refs = {_dtag: processor.put(alignments[_dtag]) for _dtag in datasets_resolution}
    dmaps_dict = processor.process_dict(
        {
            _dtag: Partial(SparseDMapStream.parallel_load).paramaterise(
            datasets_resolution[_dtag], alignment_refs[_dtag], transforms_ref, reference_frame_ref
        )
            for _dtag
    ...
    for predicted_class, count in zip(predicted_classes, counts):
        ...
        time_event_scoring_begin = time.time()
