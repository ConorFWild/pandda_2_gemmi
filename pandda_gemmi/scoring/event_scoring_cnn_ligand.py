...
from pandda_gemmi.event.event_scoring_inbuilt import EventScoringResult, LigandFittingResult, ConfomerID, \
    ConformerFittingResult, Conformers
...
class GetEventScoreCNNLigand(GetEventScoreCNNInterface):
    tag: Literal["cnn"] = "cnn_ligand"
    ...
    def __call__(self,
        ...
        for event_id, event in events.events.items():
            ...
            image_xmap = sample_xmap(dataset_xmap.xmap, sample_transform, sample_array_raw)

            sample_array_mean_map = np.copy(sample_array)
            image_mean = sample_xmap(zmap.zmap, sample_transform, sample_array_mean_map)

            sample_array_model = np.copy(sample_array)
            model_map = get_model_map(reference, zmap)
            image_model = sample_xmap(model_map, sample_transform, sample_array_model)

            # ligand_map_array = np.copy(sample_array)
            ligand_map = get_ligand_map_from_dataset(processed_dataset)
