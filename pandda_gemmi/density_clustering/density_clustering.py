...
@dataclasses.dataclass()
class Clustering(EDClusteringInterface):
...
def get_clustering_from_zmap(zmap: ZmapInterface, reference: ReferenceInterface, grid: GridInterface, contour_level: float,
                cluster_cutoff_distance_multiplier: float = 1.3) -> EDClusteringInterface:
    time_cluster_start = time.time()
...
class GetEDClustering(GetEDClusteringInterface):
    def __call__(
        self, 
        zmap: ZmapInterface, 
        ...
        ) -> EDClusteringInterface:
        return get_clustering_from_zmap(zmap, reference, grid, contour_level, cluster_cutoff_distance_multiplier)

@dataclasses.dataclass()
class Clusterings:
    clusterings: typing.Dict[DtagInterface, EDClusteringInterface]
...
def filter_size(clusterings: EDClusteringsInterface, grid: Grid, min_cluster_size: float):
        new_clusterings = {}
        for dtag in clusterings:
            clustering = clusterings[dtag]
            new_cluster_nums = list(
                filter(
                    lambda cluster_num: clustering[cluster_num].size(grid) > min_cluster_size,
                                           clustering,
                                           )
                                    )
            ...
            if len(new_cluster_nums) == 0:
                ...
                new_clusters_dict = {
                    new_cluster_num: clustering[new_cluster_num] 
                    for new_cluster_num 
                    in new_cluster_nums
                    }
...
class MergeEDClusterings:
    def __call__(self, clusterings: EDClusteringsInterface, ) -> EDClusteringsInterface:
        return merge_clusters(clusterings, )
